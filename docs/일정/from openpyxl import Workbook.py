...
def build_table_sheet(
    ...
    widths: dict,
    input_cols: set[int],
    formula_cols: dict[int, callable] | None = None,
...
def main():
    ...
    for nm, vals in lists.items():
        ...
        for j, v in enumerate(vals, start=2):
            wsL.cell(r, j, v)
        start = wsL.cell(r, 2).coordinate
        end = wsL.cell(r, 1 + len(vals)).coordinate
        wb.defined_names.append(DefinedName(nm, attr_text=f"'_LISTS'!${start}:${end}"))
        r += 2
    ...
    ws["A4"].fill = FILL_NOTE; ws["A4"].border = BORDER; ws["A4"].alignment = AL_LEFT
    ...
    ws["A7"].fill = FILL_NOTE; ws["A7"].border = BORDER; ws["A7"].alignment = AL_LEFT
    ...
    hdr = ["TR","SegmentID","Plan_Start","Plan_End","Actual_Start","Actual_End","Plan_Dur_h","Actual_Dur_h","ΔDur_h","Driver_Top1","EvidenceID_Top"]
    form = {
    ...
    hdr = ["EvidenceID","TR","TS_LT","WindowID","CauseCode","SourceType","SourceName","LocationRef","Excerpt","Weight","Reliability","LinkOrFile","Notes"]
    build_table_sheet(ws, "Evidence Log", hdr, 400,
                      {"A":16,"B":5,"C":18,"D":12,"E":16,"F":12,"G":18,"H":14,"I":52,"J":8,"K":10,"L":28,"M":30},
    ...
    ws = wb["12_GATE_EVAL"]
    hdr = ["GateID","GateValue","EventTS","METAR_Wind_kt","METAR_Gust_kt","METAR_Vis_m","NCM_Wind_kt","NCM_Wave_ft","Verdict","EvidenceID"]
    ...
    # 나머지 시트는 “빈 구조”만 필요하면 위와 동일 패턴으로 확장 가능.
    # (요청 시 TR4~TR7 Forecast/Comparison/Float, Windows/But-For/QSRA까지 완전 채움 버전도 만들어 드림)

    for s in sheets:
        wb[s].sheet_view.showGridLines = True

    wb.save("AGI_TR_Delay_Template_v3_0.xlsx")
    print("OK: AGI_TR_Delay_Template_v3_0.xlsx")
